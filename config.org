...
# Package Modernization Notes:
# - ✓ Using corfu instead of company for completions (with cape for extra sources)
# - ✓ Removed anzu in favor of built-in isearch-lazy-count
# - ✓ Replaced jade-mode with pug-mode for Pug templates
# - ✓ Using lsp-mode (supports multiple LSPs per buffer, unlike eglot)
# - ✓ Migrated to built-in treesit from external tree-sitter packages
...
;; Alternative: ace-window (more modern, actively maintained)
;; Provides numbered overlays on windows for quick jumping
...
;;           aw-background nil)
;;     ;; Make the ace-window overlays look nicer
;;     (custom-set-faces
...
#+BEGIN_SRC emacs-lisp :tangle yes
  ...
  ;; Add useful keybindings for LSP
  (with-eval-after-load 'lsp-mode
    (add-hook 'lsp-mode-hook
              (lambda ()
                (local-set-key (kbd "C-c l a") #'lsp-execute-code-action)
                (local-set-key (kbd "C-c l f") #'lsp-format-buffer)
                (local-set-key (kbd "C-c l F") #'lsp-format-region)
                (local-set-key (kbd "C-c l n") #'lsp-rename)
                (local-set-key (kbd "C-c l o") #'lsp-organize-imports)
                (local-set-key (kbd "C-c l w r") #'lsp-workspace-restart)
                (local-set-key (kbd "C-c l w s") #'lsp-workspace-shutdown))))
