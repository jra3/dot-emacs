...
GUI Emacs doesn't inherit shell PATH, so we need to source it
...
#+BEGIN_SRC emacs-lisp :tangle yes
  ...
  (let ((gopath (expand-file-name "~/go/bin")))
    (unless (member gopath exec-path)
      (add-to-list 'exec-path gopath t))
    (unless (string-match-p gopath (getenv "PATH"))
...
Hide minor mode clutter and simplify the mode line display

#+BEGIN_SRC emacs-lisp :tangle yes
  ;; Install diminish if not already available
  (use-package diminish
    :ensure t)

  ;; Hide all the noisy minor modes from mode line
  ;; These are the actual modes showing up in your setup
  (with-eval-after-load 'subword
    (diminish 'subword-mode))

  (with-eval-after-load 'diff-hl
    (diminish 'diff-hl-mode)
    (diminish 'diff-hl-flydiff-mode))
  ...
  ;; Clean up the mode line format
  ;; Remove less useful information
  (setq mode-line-percent-position nil)  ; Don't show percentage through file
  (size-indication-mode -1)              ; Don't show file size

  ;; Simplify VC (version control) display in mode line
  (setq auto-revert-check-vc-info nil)   ; Don't constantly check VC status

  ;; Customize VC display to show minimal git info (no branch name)
  ;; Shows just "Git" with a status indicator
  (defun my/simple-vc-mode-line ()
    "Return a simple VC indicator without branch name."
    (when vc-mode
      (let ((backend (vc-backend buffer-file-name)))
        (when backend
          (pcase backend
            ('Git (if (and buffer-file-name (vc-git-registered buffer-file-name))
                     (let ((state (vc-state buffer-file-name backend)))
                       (pcase state
                         ('up-to-date " Git:✓")
                         ('edited " Git:*")
                         ('added " Git:+")
  ...
  ;; Replace the VC mode line string with our simple version
  (setq-default mode-line-format
    (mapcar (lambda (elem)
              (if (equal elem '(vc-mode vc-mode))
                  '(:eval (my/simple-vc-mode-line))
                elem))
            mode-line-format))

  ;; Make the mode line more compact
  (setq mode-line-compact t)             ; Available in Emacs 28+
#+END_SRC
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (use-package ace-window
    ...
    (aw-background t)                         ; Dim background
    :custom-face
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (defvar user-temporary-file-directory "~/tmp/saves/"
    "Directory for backup and auto-save files.")
...
#+BEGIN_SRC emacs-lisp :tangle yes
  ...
  (use-package lsp-mode
    ...
    :hook ((lsp-mode           . lsp-enable-which-key-integration)
           ...
           (go-mode            . lsp-deferred)  ; Add regular go-mode
    ...
    ;; Go/gopls settings
    (lsp-go-directory-filters ["-node_modules" "-.git" "-vendor"])
  ...
  ;; Register golangci-lint-langserver as additional Go linter
  (with-eval-after-load 'lsp-mode
    (lsp-register-client
     (make-lsp-client
      ...
      :language-id "go"
      :priority -1  ; Lower priority than gopls
      :server-id 'golangci-lint-ls
      :add-on? t    ; Run alongside gopls, not instead of
      :initialization-options
      '((command . ["golangci-lint" "run" "--out-format=json"])))))

  ;; ESLint via lsp-mode (runs alongside typescript-language-server)
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (use-package org
    ...
    :hook ((org-mode . (lambda () (electric-indent-local-mode -1)))
           (org-capture-before-finalize . (lambda () (save-buffer)))
    ...
    (setq org-capture-templates
          '(("t" "Todo" entry (file+headline "~/org/gtd.org" "Inbox")
            ...
            ("j" "Journal" entry (file+olp+datetree "~/org/journal.org")
             "* %?\n  %U\n  %i")
            ("5" "5 Minute Journal")
            ("5m" "Morning Entry" entry (file+olp+datetree "~/org/5-min-journal.org")
             "* Morning\n  I am grateful for...\n  - %?\n  - \n  - \n\n  What will I do to make today great?\n  - \n  - \n  - \n\n  I am ...")
            ("5e" "Evening Entry" entry (file+olp+datetree "~/org/5-min-journal.org")
...
#+BEGIN_SRC emacs-lisp :tangle yes
  ...
  ;; IMPORTANT: Force Go files to use go-ts-mode
...
#+BEGIN_SRC emacs-lisp :tangle yes
  ;; Go indentation settings (tabs, width 4)
  (defun my/go-mode-setup ()
    (setq indent-tabs-mode t)
    (setq tab-width 4)
    ;; Use LSP for formatting on save (gopls uses gofumpt if configured)
