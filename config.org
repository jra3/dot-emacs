...
#+BEGIN_SRC emacs-lisp :tangle yes
  ...
  (use-package lsp-mode
    ...
    ;; Go/gopls specific settings to prevent "no views" error
    (lsp-go-gopls-server-path "/Users/john/go/bin/gopls")
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (use-package go-mode
    :ensure t
    ;; DO NOT use :mode - let auto-mode-alist handle it
    :hook ((go-mode . (lambda ()
                        (setq indent-tabs-mode t)
                        (setq tab-width 4)))
           ;; Also configure go-ts-mode
           (go-ts-mode . (lambda ()
                          (setq indent-tabs-mode t)
                          (setq tab-width 4))))
    :config
